document.addEventListener("DOMContentLoaded", function () {
  ...
  try {
    const notifCard = document.getElementById("notifSettingsCard");
    if (notifCard) {
      var goNotif = function () {
        window.location.href = "powiadomienia.html?from=more";
      };
      notifCard.style.cursor = "pointer";
      notifCard.addEventListener("click", goNotif);
      notifCard.addEventListener("keydown", function (e) {
        if (e && (e.key === "Enter" || e.key === " ")) {
          e.preventDefault();
          goNotif();
        }
